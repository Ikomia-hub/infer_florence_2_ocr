...
# --------------------
class InferFlorence2OcrWidget(core.CWorkflowTaskWidget):

    def __init__(self, param, parent):
        core.CWorkflowTaskWidget.__init__(self, parent)

        if param is None:
            self.parameters = InferFlorence2OcrParam()
        else:
...
# --------------------
class InferFlorence2OcrWidgetFactory(dataprocess.CWidgetFactory):

    def __init__(self):
        dataprocess.CWidgetFactory.__init__(self)
        # Set the algorithm name attribute -> it must be the same as the one declared in the algorithm factory class
        self.name = "infer_florence_2_ocr"

    def create(self, param):
        # Create widget object
        return InferFlorence2OcrWidget(param, None)
